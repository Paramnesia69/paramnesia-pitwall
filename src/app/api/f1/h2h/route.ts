...
export async function GET() {
  ...
  // State per pair: { name, d1Wins, d2Wins, rounds, d1Name, d2Name }
  ...
  for (const race of races) {
    // Build a map: driverRef → qualifying position for this race
    ...
    for (const [cId, ref1, ref2] of TEAM_PAIRS) {
      ...
      if (p1 == null || p2 == null) continue; // one or both DNS/absent
      const entry = stats.get(cId)!;
      entry.rounds++;
      if (p1 < p2) entry.d1Wins++;
      else if (p2 < p1) entry.d2Wins++;
      // exact tie counts for neither (rare/impossible in real qualifying but safe)
    }
  }

  // Enrich with display names from qualifying results
  ...
  for (const [cId, ref1, ref2] of TEAM_PAIRS) {
    ...
    if (s.rounds === 0) continue; // no data yet
    result.push({
      team: driverNames.get(ref1) ? // use first driver's constructor display name fallback
  ...
  // Sort by total rounds descending (teams that've raced most first)
