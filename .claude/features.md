...
- `gtwce.png`, `wec.png`, `elms.png` have non-transparent backgrounds — at opacity 0.18 with `maskImage: radial-gradient(ellipse at center, black 50%, transparent 85%)` the rectangular background is barely visible; acceptable. Never use `mixBlendMode: screen` — iOS Safari drops it inside stacking contexts (backdrop-filter + overflow:hidden + border-radius).
- **Series watermarks** (HeroCard / EventCard / EventDetailOverlay) — **no filter**, natural brand colors, opacity 0.12–0.18, radial-gradient `maskImage`. Porsche SVG only: `brightness(0) invert(1)`. Circuit maps keep `brightness(3) contrast(1.2) saturate(0.5)` to extract line-art.
