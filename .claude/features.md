...
### WEC Race-Intelligence Layer (2026-06-14)
Full-screen timing board (`WECTimingBoard`, "Full board" button in `WECTimingPanel`) + extras, all from the live Griiip bootstrap (`normalizeGriiipBootstrap`):
- **Live commentator** — `commentatorPhraseLatest` → narrative ticker (board top + panel) with a play button for the official MP3 (`audioUrl`). Updates each lap.
- **Battle highlight** — `runningBattles` → `entry.inBattle`: orange left-edge + BATTLE badge on rows in an active on-track fight.
- **Gap-trend arrows** — `gapTrends` (Decreasing→closing ▾ green / Increasing→growing ▴) shown in the INT cell.
- **Striking distance** — `strikingDistances.strikingDistanceLaps` → `entry.strikeLaps`: green "◢N.NL" badge = est. laps to catch the car ahead.
- **In-pit** — `carLocations` zone (`'Pit'`) → `entry.inPit`: amber PIT badge.
- **Race Log drawer** — `raceLogFirstPage.items` → readable feed (overtakes, pits, fastest laps, penalties/RC messages, driver changes); secondary car numbers resolved from participants.
- **NOT available (verified against the live feed, do not re-investigate):** RPM / gear / throttle / brake (absent everywhere); continuous speed (only `latestSpeedTraps` point readings, empty this race); smooth GPS car dots / track polyline (`carLocations` is a coarse zone only, `hasTrackInfo:false`) — the moving-dot map needs a persistent SignalR socket, which our Vercel-serverless/no-websocket architecture can't hold. Even FIA WEC+ public timing shows none of these. So **no gauges** — a needle with no data would be fake (violates real-data-only).

### Multi-Series Live Timing — F1 + WEC + ELMS (2026-06-14)
The live event panel lights up for any live event in a series we have a real feed for; each gets the telemetry its source exposes:
- **F1** — `F1TimingPanel` (OpenF1), any live F1 event.
- **WEC** — `WECTimingPanel` (Griiip live + Al Kamel fallback), any live WEC event.
- **ELMS** — same `WECTimingPanel` pointed at `/api/elms/timing`. Al Kamel-only (ELMS has no Griiip backend). `lib/elmsTiming.ts` auto-discovers the live round by scraping the Al Kamel tree-menu homepage (`parseLatestRaceBase` → latest `…/{timestamp}_Race` folder), then probes the live root + `Hour N` snapshots (4h races). Four ELMS classes: LMP2 / LMP2 Pro/Am / LMP3 / LMGT3. Reuses `parseWecClassification` (identical single-field Al Kamel schema). Mounts for any live/starting_soon ELMS event. Note: ELMS gets the leaderboard/board/weather columns, but NOT the Griiip-only race-intelligence layer (commentator/battle/trend/strike/pit/race-log) — those are absent from the CSV and gracefully no-op.
- **Panel is now generic**: `WECTimingPanel` + `WECTimingBoard` take `endpoint` / `providerName` / `providerUrl` / `boardTitle` props (default to WEC). `CLASS_ORDER` (`wecTiming.ts`) + `CLASS_COLOR` (panel + board) extended with LMP2 Pro/Am + LMP3.
- **Next**: IMSA (also Al Kamel — `imsa.alkamelsystems.com` confirmed live, but needs 2026 season-folder path discovery + its richer `DRIVERn_FIRSTNAME/SECONDNAME` schema, already handled by the parser). MotoGP has **no public live-timing REST feed** (PulseLive is post-session only; `timing.motogp.com` is a closed WebSocket → incompatible with our no-socket serverless arch).
