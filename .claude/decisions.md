...
| Logo strategy | SVG preferred; PNG/JPG with blend mode | SVGs transparent by default; non-transparent PNGs need screen blend + radial mask |
...
- Non-transparent logos: always apply `filter: grayscale(1) contrast(2) brightness(3)`, `mixBlendMode: screen`, and `maskImage: radial-gradient(ellipse at center, black 30%, transparent 70%)`
