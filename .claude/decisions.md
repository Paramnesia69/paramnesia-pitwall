...
| Multi-series live timing | Mount the live panel for ANY live event in a series we have a real feed for (F1/WEC/ELMS); each gets the telemetry its source has | User: "this kind of dashboard… for any series we can, starting with F1 and WEC. each gets the telemetry it can." Panel gate = `series` + `state in {live, starting_soon}`, no per-circuit hardcoding. |
| ELMS live data | Al Kamel CSV only, auto-discover the live round | ELMS has **no Griiip backend** (WEC-exclusive). Round changes per event, so `parseLatestRaceBase` (`elmsTiming.ts`) scrapes the Al Kamel tree-menu homepage (it inlines every session path) for the highest-numbered `…/{timestamp}_Race` folder, then probes the live root + `Hour N` snapshots (4h races). Reuses `parseWecClassification`; only `CLASS_ORDER`/`CLASS_COLOR` gained LMP2 Pro/Am + LMP3. ELMS gets leaderboard/board/weather, NOT the Griiip-only race-intelligence layer. |
| MotoGP live timing | None — post-session results only | No public live-timing REST feed. PulseLive (`api.motogp.pulselive.com`) serves only finished classifications; `timing.motogp.com` is a closed WebSocket/Meteor app — same persistent-socket problem as Al Kamel's realtime feed, incompatible with our no-websocket serverless arch. Don't re-investigate live timing. |
...
| Endurance road marker | Real LMP prototype **PNG** (`/lemans-prototype.png`), not the SVG | User: the synthetic SVG car "is clearly not a hypercar". Sourced a real side-profile prototype. The SVG `Hypercar` stays only for the `racing` phase-icon fallback. |
| Cutting the car from white bg | Flood-fill bg inward from all 4 edges (keeps enclosed white *car* pixels) → **morphological opening** (`MinFilter`×2 then `MaxFilter`×2) to erase the dither halo → trim shadow + recrop | The source is a **16-colour dithered** PNG with an **opaque** white/grey bg (not transparent — the "checkerboard" was the dither). Global colour-key punches holes in the white car; flood-fill-from-edges + opening is the only clean separation. Verify by compositing onto the dark road colour, not on white. |
| Asset filename versioning | New filename when an image's *content* changes (here `endurance-car.png` → `lemans-prototype.png`) | `sw.js` serves images **cache-first** — same path = stale forever. Same trick the favicon uses. |
| WEC class colours | Official ACO/ELMS plate palette, **fixed per class at all times**: Hypercar red / LMP2 blue / LMGT3 green | User: colours were "flapping" when inactive tabs went neutral blue. Each class now always wears its own colour (active brighter, inactive dimmed) — drives both the tab banner and the leaderboard accent. |
| Live session row / Live dot / YouTube links | Row + name = **series accent** (WEC blue); `pw-live-dot` stays **red**; YouTube watch links use the series accent (not YouTube red) | User wanted the WEC context to read blue, but the universal "live" signal stays red. Series-accent colouring keeps it consistent across series (F1 → red, WEC → blue, …). |
